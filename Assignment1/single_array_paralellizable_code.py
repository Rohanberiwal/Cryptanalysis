...
def apply_sub_bytes(matrix):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(lambda r, c: S_BOX[matrix[r][c] >> 4][matrix[r][c] & 0x0F], r, c) for r in range(4) for c in range(4)]
        for idx, future in enumerate(futures):
            r, c = divmod(idx, 4)
            matrix[r][c] = future.result()
    return matrix

# Shift the rows of the state matrix (sequential)
...
# Mix the columns of the state matrix (parallelizable)
def apply_mix_columns(matrix):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(lambda col: [
            gmul(matrix[0][col], 2) ^ gmul(matrix[1][col], 3) ^ gmul(matrix[2][col], 1) ^ gmul(matrix[3][col], 1),
            gmul(matrix[0][col], 1) ^ gmul(matrix[1][col], 2) ^ gmul(matrix[2][col], 3) ^ gmul(matrix[3][col], 1),
            gmul(matrix[0][col], 1) ^ gmul(matrix[1][col], 1) ^ gmul(matrix[2][col], 2) ^ gmul(matrix[3][col], 3),
            gmul(matrix[0][col], 3) ^ gmul(matrix[1][col], 1) ^ gmul(matrix[2][col], 1) ^ gmul(matrix[3][col], 2)
        ], col) for col in range(4)]
        
        for idx, future in enumerate(futures):
            col_results = future.result()
            for row in range(4):
                matrix[row][idx] = col_results[row]
    return matrix

# XOR the state with the round key (parallelizable)
def xor_with_round_key(matrix, round_key):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(lambda r, c: matrix[r][c] ^ round_key[r][c], r, c) for r in range(4) for c in range(4)]
        for idx, future in enumerate(futures):
            r, c = divmod(idx, 4)
            matrix[r][c] = future.result()
    return matrix

# Expand the key for multiple rounds (this can also be parallelized in chunks)
def generate_round_keys(key):
    ...
    # Parallelize parts of key expansion
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for col in range(4, 44):
            ...
            if col % 4 == 0:
                ...
                temp[0] ^= RCON[(col // 4) - 1]  # Apply RCON from the new array
...
# Convert the state matrix to a hexadecimal string
...
def encrypt_aes(plaintext, key):
    ...
    return matrix_to_hex(state_matrix)
...
def main():
    ...
    print("Output of AES 128 (hex):", encrypted_text)
