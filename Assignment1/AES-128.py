...
def key_expansion(key):
    ...
    key_schedule = [[0] * 4 for _ in range(4 * (10 + 1))]

    
    for col in range(4):
        for row in range(4):
            key_schedule[col][row] = key_symbols[row + 4 * col]

    
    for col in range(4, 4 * (10 + 1)):
        tmp = [key_schedule[col - 1][row] for row in range(4)]
...
def encrypt(plaintext, key, rounds=10):
    ...
    for rnd in range(1, rounds):
        ...
        state = add_round_key(state, key_schedule[4 * rnd: 4 * (rnd + 1)])
    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, key_schedule[4 * rounds:])
    encrypted = ''
    for r in range(4):
        for c in range(4):
            encrypted += chr(state[r][c])
    return encrypted
...
if _name_ == "_main_":
    ...
    rounds = 10
    encrypted_text = encrypt(plaintext, key, rounds)
